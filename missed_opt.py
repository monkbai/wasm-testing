...
def main():
    ...
    while file_idx < 1001:
        ...
        # get_one_csmith(c_path)
        glob_correct, func_correct, glob_perf, func_perf = trace_consistency.trace_check(c_path, clang_opt_level='-O3', emcc_opt_level='-O3')
        output1, status = utils.run_single_prog("./missopt_cases/testcases/test{}.out".format(file_idx))
        output2, status = utils.run_single_prog("node ./missopt_cases/testcases/test{}.js".format(file_idx))
        if len(glob_correct) == 0 and len(func_correct) == 0:
            if len(glob_perf) != 0 or len(func_perf) != 0:
                status, output = utils.cmd(
                    "cp ./missopt_cases/testcases/test{}.c ./missopt_cases/bug_cases/test{}.c".format(file_idx, file_idx))

        status, output = utils.cmd("rm ./missopt_cases/testcases/test{}.c.*".format(file_idx))
        status, output = utils.cmd("rm ./missopt_cases/testcases/test{}.out".format(file_idx))
        status, output = utils.cmd("rm ./missopt_cases/testcases/*.js".format(file_idx))
        status, output = utils.cmd("rm ./missopt_cases/testcases/*.wasm".format(file_idx))
        status, output = utils.cmd("rm ./missopt_cases/testcases/*.wat".format(file_idx))
        status, output = utils.cmd("rm ./missopt_cases/testcases/*.dwarf".format(file_idx))
        status, output = utils.cmd("rm ./missopt_cases/testcases/*.trace".format(file_idx))
        file_idx += 1
    print(file_idx)
