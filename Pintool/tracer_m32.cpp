...
VOID RecordMemWrite(VOID * ip, VOID * mem_addr, USIZE mem_size)
{
    ...
    if (mem_size == 1){
        ...
        fprintf(trace,"V: unimplemented mem_size: %u\n", mem_size);
    }
}

VOID AfterCall(VOID * ip, ADDRINT eax, ADDRINT callee_addr){
    std::string ins_str = str_of_ins_at[(ADDRINT)ip];
    ...
    if (ret_func_addr.find(callee_addr) != ret_func_addr.end()){  // only print return value of the target functions
        ...
        fprintf(trace,">%p R: %p\n", (VOID *)callee_addr, (void *)eax);
    }
}

VOID RecordArgsm32(VOID * ip, ADDRINT esp){
    if (func2param.find((uint64_t)ip) == func2param.end()){
        return; // no argument
    }
    fprintf(trace,">%p\n", ip); // print function start address
    // If the argument is a pointer: treat it as a int32 and print the pointer value
    ADDRINT current_addr = esp + 4;  // return addr -> 4 bytes, push ebp in stack -> 4 bytes
    for (std::list<uint32_t>::iterator it = func2param[(uint64_t)ip].begin(); it != func2param[(uint64_t)ip].end(); ++it) {
        if (*it == 1){  // 1 byte
            fprintf(trace, "P: 0x%x\n", (unsigned int)(*(uint8_t *)(current_addr)));
            current_addr += 1;
        }
        else if (*it == 2){
            fprintf(trace, "P: 0x%x\n", (unsigned int)(*(uint16_t*)(current_addr)));
            current_addr += 2;
        }
        else if (*it == 4){
            fprintf(trace, "P: 0x%x\n", (unsigned int)(*(uint32_t*)(current_addr)));
            current_addr += 4;
        }
        else if (*it == 8){
            fprintf(trace, "P: 0x%llx\n", (long long unsigned int)(*(uint64_t *)(current_addr)));
            current_addr += 8;
        }
        else{
...
VOID Instruction(INS ins, VOID *v)
{
    ...
    if (func_addr.find(ins_addr) != func_addr.end()){
        ...
        // RecordArgsm32: print the arguments according to the param_file
        INS_InsertPredicatedCall(
            ins, IPOINT_BEFORE, (AFUNPTR)RecordArgsm32,
            IARG_INST_PTR,
    ...
    if (INS_IsRet(ins)){
        ...
        for (auto i = ret_func_addr.begin(); i != ret_func_addr.end(); i++){
            ...
            if (high_addr >= ins_addr && ins_addr > low_addr){
                ...
                IARG_ADDRINT, (uint32_t)low_addr,
                IARG_END);
